...
static const int16_t deemph_taps[2][3] = {{14430, 14430, -3909},
                                          {10571, 10571, -11626}};
...
void inline rx_dsp ::iq_imbalance_correction(int16_t &i, int16_t &q) {
  if (iq_correction) {
    ...
    if (++index == 512) {
      ...
      theta1_filtered =
          theta1_filtered - (theta1_filtered >> 5) + (-theta1 >> 5);
      theta2_filtered =
          theta2_filtered - (theta2_filtered >> 5) + (theta2 >> 5);
      theta3_filtered =
          theta3_filtered - (theta3_filtered >> 5) + (theta3 >> 5);

      // try to constrain square to less than 32 bits.
      // Assue that i/q used full int16_t range.
      // Accumulating 512 samples adds 9 bits of growth, so remove 18 after
      // square.
...
uint16_t __not_in_flash_func(rx_dsp ::process_block)(
    uint16_t samples[], int16_t audio_samples[], ring_buffer_t *iq_samples) {
  ...
  for (uint16_t idx = 0; idx < adc_block_size; idx++) {
    ...
    int16_t i =
        ((idx & 1) ^ 1 ^ swap_iq) * raw_sample;  // even samples contain i data
    int16_t q =
        ((idx & 1) ^ swap_iq) * raw_sample;  // odd samples contain q data
...
void __not_in_flash_func(rx_dsp ::frequency_shift)(int16_t &i, int16_t &q) {
  ...
  const int16_t rotation_i =
      sin_table[(scaled_phase + 512u) & 0x7ff];  // 32 - 21 = 11MSBs
  const int16_t rotation_q = -sin_table[scaled_phase];

  phase += frequency;
  // truncating fractional bits introduces bias, but it is more efficient to
  // remove it after decimation
  int32_t bias = (1 << 14);
  const int16_t i_shifted =
      (((int32_t)i * rotation_i) - ((int32_t)q * rotation_q) + bias) >> 15;
  const int16_t q_shifted =
      (((int32_t)q * rotation_i) + ((int32_t)i * rotation_q) + bias) >> 15;
...
bool __not_in_flash_func(rx_dsp ::decimate)(int16_t &i, int16_t &q) {
  ...
  if (decimate_count >= cic_decimation_rate) {
    ...
    // remove bit growth, but keep some extra bits since noise floor is now
    // lower
...
int16_t __not_in_flash_func(rx_dsp ::demodulate)(int16_t i, int16_t q,
  ...
  {
    cw_sidetone_phase +=
        cw_sidetone_frequency_Hz * 2048 * decimation_rate / adc_sample_rate;
...
void rx_dsp ::set_deemphasis(uint8_t deemph) { deemphasis = deemph; }
...
void rx_dsp ::set_frequency_offset_Hz(double offset_frequency) {
  ...
  frequency = ((double)(1ull << 32) * offset_frequency) * cic_decimation_rate /
              (adc_sample_rate);
}
...
void rx_dsp ::set_swap_iq(uint8_t val) { swap_iq = val; }

void rx_dsp ::set_iq_correction(uint8_t val) { iq_correction = val; }
...
void rx_dsp ::set_gain_cal_dB(uint16_t val) {
  ...
  s9_threshold = full_scale_signal_strength *
                 powf(10.0f, (S9 - full_scale_dBm + amplifier_gain_dB) / 20.0f);
}
...
int16_t rx_dsp ::get_signal_strength_dBm() {
  ...
  const float signal_strength_dBFS =
      20.0 * log10f((float)signal_amplitude / full_scale_signal_strength);
  return roundf(full_scale_dBm - amplifier_gain_dB + signal_strength_dBFS);
}

s_filter_control rx_dsp ::get_filter_config() { return capture_filter_control; }

static int16_t cic_correct(int16_t fft_bin, int16_t fft_offset,
                           uint16_t magnitude) {
  ...
  uint32_t adjusted_magnitude =
      ((uint32_t)magnitude * cic_correction[unsigned_fft_bin]) >> 8;
...
static inline uint8_t fft_shift(uint8_t bin) { return bin ^ 0x80; }
...
void rx_dsp ::get_spectrum(uint8_t spectrum[], uint8_t &dB10) {
  ...
  for (uint16_t i = 0; i < 256; ++i) {
    const uint16_t magnitude =
        cic_correct(freq_bin(i), capture_filter_control.fft_bin, capture[i]);
  ...
  for (uint16_t i = 0; i < 256; i++) {
    ...
    if (magnitude == 0) {
      ...
      const float normalised =
          255.0f * (log10f(magnitude) - logmin) / (logmax - logmin);
